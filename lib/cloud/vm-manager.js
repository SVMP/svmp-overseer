                    ...
                    for (var i = 0; i < obj.length; i++) {
                        ...
                        sess.removeQ()
                            .then(function (result) {
                                svmp.logger.verbose("Removed session for '%s'", result.username)
                            }).catch(function (err) {
                                svmp.logger.error("Couldn't remove session:", err.message);
                            }).done();
...
function getExpiredVmSessions(req, res) {
    var query = {
        'lastAction': {
            '$lt': toDate(svmp.config.get('settings:vm_idle_ttl')).seconds.ago,
