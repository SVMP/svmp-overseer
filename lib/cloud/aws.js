...
aws.init = function () {
    var awsInfo = svmp.config.get('aws');
    if (!awsInfo) {
        svmp.logger.error("Failed to initialize AWS: cloud_platform set to 'aws', but no AWS config (aws) found!");
        process.exit(1);
    }
...
aws.createAndStartVM = function (user, image, flavor) {
    ...
    var vmObj = {
        ...
        flavor: flavor,
        image: image,
        zone: svmp.config.get('aws:availabilityZone')
    };
...
aws.createVolumeForUser = function (user) {
    ...
    var params1 = {
        AvailabilityZone: svmp.config.get('aws:availabilityZone'),
        SnapshotId: svmp.config.get('new_vm_defaults:goldsnapshotId')
    };
