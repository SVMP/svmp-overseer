...
openstack.init = function () {
    var openstackInfo = svmp.config.get('openstack');
    if (!openstackInfo) {
        svmp.logger.error("Failed to initialize OpenStack: cloud_platform set to 'openstack', but no OpenStack config (openstack) found!");
        process.exit(1);
    }
...
openstack.setUpUser = function (userObj) {
    ...
    var flvr = svmp.config.get('new_vm_defaults:vmflavor');
    var user = userObj;
    var img = svmp.config.get('new_vm_defaults:images')[user.device_type];
    ...
    if (user.vm_ip && user.vm_ip.length > 0) {
        ...
        openstack.createAndStartVM(user, img, flvr)
            ...
            .then(function (updatedUser){
                var un = updatedUser.username;
                delete updatedUser.username;

                svmp.User.update({username: un}, updatedUser, function (err) {
                    if (err) {
                        deferred.reject(new Error('Failed updating user: ' + un));
                    } else {
                        deferred.resolve({user: updatedUser});
                    }
                });
            ...
            .catch(function (err) {
                svmp.logger.error("Error creating user:", err.message);
            })
            .done();
    }
...
openstack.createAndStartVM = function (user, image, flavor) {
    ...
    openstack.computeClient.createServer(vmObj, function (err, server) {
        var poll = svmp.config.get('new_vm_defaults:pollintervalforstartup');
        ...
        if (err) {
            ...
            server.setWait({status: server.STATUS.running}, poll, function (err) {
                if (err) {
                    ...
                    if (svmp.config.isEnabled('new_vm_defaults:use_floating_ips')) {
...
openstack.createVolumeForUser = function (user) {
    ...
    var goldSize = svmp.config.get('new_vm_defaults:goldsnapshotSize');
    var goldId = svmp.config.get('new_vm_defaults:goldsnapshotId');
...
openstack.assignFloatingIpToVM = function (user) {
    ...
    if (svmp.config.isEnabled('new_vm_defaults:use_floating_ips')) {
        ...
        var pool = svmp.config.get('new_vm_defaults:floating_ip_pool'); // optional
