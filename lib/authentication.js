...
exports.makeToken = function (userInfo) {
    return jwt.sign(userInfo, getPrivKey(), {algorithm: svmp.config.get("settings:jwt_signing_alg")});
};
...
function getPrivKey() {
    if (privKey === null) {
        var pass = svmp.config.get('settings:tls_private_key_pass');
        var file = svmp.config.get('settings:tls_private_key');
...
function getPubKey() {
    if (pubKey === null) {
        pubKey = fs.readFileSync(svmp.config.get('settings:tls_certificate'));
    }
    return pubKey;
}
