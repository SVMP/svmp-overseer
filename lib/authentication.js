    ...
    if (token) {
        jwt.verify(token, getPubKey(), function (err, decoded) {
            if (err) {
                ...
                res.error = 'Token has no subject specified';
    ...
    if (token) {
        jwt.verify(token, getPubKey(), function (err, decoded) {
            if (err) {
                ...
                res.error = 'Incorrect token role \'' + decoded.role + '\' for requested action';
                res.json(401, {msg: 'wrong role'});
            } else {
                req.user = decoded;

                svmp.logger.info('%s, %s, %s, %s', new Date().toString(), req.ip, decoded.sub, req.path);
